...
function matchCellWidths(els) {
	...
	els.find('> *').each(function(i, innerEl) {
		var innerWidth = $(innerEl).outerWidth(true);
...
function getScrollbarWidths(container) {
	...
	var innerRight = innerLeft + inner.outerWidth(true);
...
Popover.prototype = {
	...
	position: function() {
		...
		if (options.viewportConstrain !== false) {
			...
			left = Math.min(left, viewportLeft + viewportEl.outerWidth(true) - width - this.margin);
...
GridCoordMap.prototype = {
	...
	computeBounds: function() {
		...
		if (this.containerEl) {
			...
			this.maxX = containerOffset.left + this.containerEl.outerWidth(true);
	...
	computeScrollBounds: function() {
		...
		if (el) {
			...
			this.scrollBounds = {
				...
				right: offset.left + el.outerWidth(true)
			};
		}
	},
...
$.extend(DayGrid.prototype, {
	...
	showSegPopover: function(date, cell, moreLink, segs) {
		...
		if (view.opt('isRTL')) {
			options.right = moreWrap.offset().left + moreWrap.outerWidth(true) + 1; // +1 to be over cell border
		}
		else {
